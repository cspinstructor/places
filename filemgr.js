...
const getAllData = () => {
  return new Promise((resolve, reject) => {
    MongoClient.connect(database, {useNewUrlParser: true}, (err, client) => {
      ...
      db.collection('placesappcollection').find().toArray().then((docs) => {
        resolve(docs);
      }, (err) => {
  ...
  return new Promise((resolve, reject) => {
    MongoClient.connect(database, {useNewUrlParser: true}, (err, client) => {
      ...
      for (var i=0; i<length; i++) {
        db.collection('placesappcollection').insertOne(newdata[i],(err, result) => {
          if (err) {
...
const deleteAll = () => {
  return new Promise((resolve, reject) => {
    MongoClient.connect(database, {useNewUrlParser: true}, (err, client) => {
      ...
      console.log('Connected to MongoDB server');
      const db = client.db('placesapp');
      db.collection('placesappcollection').remove({}).then((result) => {
